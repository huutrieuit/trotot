...
function LoginForm({ ssoOnly }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authErr) {
      if (authErr.message.toLowerCase().includes("email not confirmed")) {
        ...
        setError("Email chưa được xác nhận. Kiểm tra hộp thư và bấm vào link xác nhận.");
      } else {
  ...
  return (
    <div className="w-full max-w-md">
      ...
      {/* Email/password form — ẩn khi ssoOnly */}
      {!ssoOnly && (
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {error && (
              <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-2.5 space-y-2">
                <p>{error}</p>
                {emailUnconfirmed && form.email && (
                  resent ? (
                    <div className="flex items-center gap-1.5 text-green-700 text-xs font-medium">
                      <MailCheck size={14} />
                      Đã gửi lại email xác nhận. Kiểm tra hộp thư (kể cả Spam).
                    </div>
                  ) : (
                    <button
                      type="button"
                      ...
                      disabled={resending}
                      className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:underline disabled:opacity-60"
                    >
...
export default function DangNhapForm({ ssoOnly }: Props) {
  return (
